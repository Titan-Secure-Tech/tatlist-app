...
async function scrapeLuckyProductIds() {
  ...
  try {
    ...
    console.log(`Found ${pageProductIds.length} unique products on page 1`)
    
    // Try a few more pages manually
    for (let pageNum = 2; pageNum <= 5; pageNum++) {
      try {
        ...
        const moreProductIds = await page.evaluate(() => {
          const productLinks = document.querySelectorAll('a[href*="/products/"]')
          const ids = new Set<string>()
        ...
        const newIds = moreProductIds.filter(id => !productIds.has(id))
        newIds.forEach(id => productIds.add(id))
        console.log(`Found ${newIds.length} new products on page ${pageNum}`)
    ...
    console.log('\nSample product IDs:')
    productIdArray.slice(0, 10).forEach(id => console.log(`  ${id}`))
...
scrapeLuckyProductIds()
  .then(ids => {
    console.log('\nScraping complete!')
    console.log('Next step: Test these IDs with the Lucky Supply API')
  })
  .catch(console.error)