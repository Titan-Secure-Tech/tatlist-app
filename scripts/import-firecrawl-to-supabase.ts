...
async function importFireCrawlProducts() {
  ...
  try {
    ...
    const supabasePath = path.join(process.cwd(), 'data', 'lucky-products-supabase.json')
    ...
    try {
      ...
      products = completeResults.products.map((product: FireCrawlProduct) => ({
        sku: product.sku,
        name: product.name,
    ...
    console.log(`  • Attachments: ${sampleProduct.attachments?.length || 0}`)
    ...
    for (let i = 0; i < products.length; i += batchSize) {
      ...
      const batchImages = batch.reduce((sum, p) => sum + (p.images?.length || 0), 0)
      const batchAttachments = batch.reduce((sum, p) => sum + (p.attachments?.length || 0), 0)
      ...
      console.log(
        `   📊 Progress: ${totalInserted}/${products.length} products (${Math.round((totalInserted / products.length) * 100)}%)`
      )
    }
    ...
    if (!countError && dbProducts) {
      ...
      console.log(
        `• Products with images: ${productsWithImages} (${Math.round((productsWithImages / dbProducts.length) * 100)}%)`
      )

      // Count products with attachments
      const productsWithAttachments = dbProducts.filter(
        p => p.attachments && p.attachments.length > 0
      ).length
      console.log(
        `• Products with attachments: ${productsWithAttachments} (${Math.round((productsWithAttachments / dbProducts.length) * 100)}%)`
      )
    }
...
async function showImportPreview() {
  ...
  try {
    ...
    products.slice(0, 5).forEach((product, i) => {
      ...
      console.log(
        `   SKU: ${product.sku} | Price: $${product.price} | Images: ${product.images.length} | Attachments: ${product.attachments.length}`
      )
    })
