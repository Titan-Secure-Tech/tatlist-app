...
export function SquarePaymentForm({ amount, onSuccess, isProcessing }: SquarePaymentFormProps) {
  ...
  useEffect(() => {
    ...
    const initializePayments = async () => {
      try {
        ...
        const paymentsInstance = (window as unknown as { Square: { payments: (appId: string, locId: string) => unknown } }).Square.payments(applicationId, locationId)
        setPayments(paymentsInstance)

        // Initialize Card payment method
        const cardInstance = await (paymentsInstance as { card: () => Promise<{ attach: (selector: string) => Promise<void> }> }).card()
        await cardInstance.attach('#card-container')
        setCard(cardInstance)

        // Check if Google Pay is available
        try {
          const googlePayRequest = (paymentsInstance as { paymentRequest: (req: unknown) => unknown }).paymentRequest({
            countryCode: 'US',
            currencyCode: 'USD',
            total: {
          ...
          const googlePayInstance = await (paymentsInstance as { googlePay: (req: unknown) => Promise<{ attach: (selector: string) => Promise<void> }> }).googlePay(googlePayRequest)
  ...
  const handleCardPayment = async () => {
    ...
    try {
      const result = await (card as { tokenize: () => Promise<{ status: string; token?: string; errors?: Array<{ message: string }> }> }).tokenize()
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          <div 
            id="card-container" 
            className="min-h-[90px] p-3 border rounded-md"
          />
        </div>
